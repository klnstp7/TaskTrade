...
namespace Peacock.PEP.Data.Entities
{
    public partial class SummaryData
    {
        ...
        internal class SummaryDataConfig : EntityConfig<SummaryData>
        {
            internal SummaryDataConfig()
            {
                base.ToTable("SummaryData");
                base.HasKey(x => x.Id);
                HasRequired(x => x.Project).WithRequiredDependent(x => x.SummaryData).Map(x => x.MapKey("ProjectID"));
            }
        }
    }
}
